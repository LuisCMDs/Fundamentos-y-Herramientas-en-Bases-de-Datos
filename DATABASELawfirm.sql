...
## SE CREAN LAS TABLAS PARA LA BASE DE DATOS LAWFIRM y
-- Paso 2 (se aplica ON DELETE CASCADE)
...
CREATE TABLE tipos_material (
	ID INT AUTO_INCREMENT PRIMARY KEY,
	Tipo VARCHAR(25) NOT NULL -- Si es un libro, revista o artículo
);

CREATE TABLE formatos_material (
	ID INT AUTO_INCREMENT PRIMARY KEY,
	Formato VARCHAR(20) NOT NULL -- Si es fisico o digital 
);
...
## Se aplica ON DELETE CASCADE porque las actuaciones pertenecen al caso
...
## Lass audiencas tambien pertenecen al caso entonces se aplica ON DELETE CASCADE
...
## Idiomas no puede existir sin abogado (ON DELETE CASCADE) 
-- Los idiomas que habla el abogado (multivaluado)
...
## La asignacion no existe si se elimina el caso o el abogado (ON DELETE CASCADE)
-- Los abogados secundarios que se presenten en el caso (M:M)
CREATE TABLE caso_abogado (
	ID INT AUTO_INCREMENT PRIMARY KEY,
	ID_caso INT NOT NULL,
	ID_abogado INT NOT NULL,
	Rol VARCHAR(20), -- Principal o secundario 
	FOREIGN KEY (ID_caso) REFERENCES casos(ID) ON DELETE CASCADE,
	FOREIGN KEY (ID_abogado) REFERENCES abogados(ID) ON DELETE CASCADE 
);

## Si no hay audiencia, ¿Dónde estarán los participantes? (ON DELETE CASCADE)
-- Los que van a participar en la audiencia (internos y externos) 
...
## No hay vínculo si se elimina la factura o el caso (ON DELETE CASCADE) 
-- Casos que se incluyan en una factura (M:M)
...
## Los detalles pertenecen a la factura (ON DELETE CASCADE)
-- Detalles de los servicios de la factura (horas por abogado, tarifa) 
...
-- Los documentos que se asocien a una actuacion (M:M)
...
## No hay destinatarios si NO hay documento (ON DELETE CASCADE)
-- Destinatarios (Multivaluado)
CREATE TABLE destinatario_documento (
	ID INT AUTO_INCREMENT PRIMARY KEY,
	ID_documento INT,
	ID_abogado INT, -- NULL si es externo
	Destinatario VARCHAR(50), -- Nombre si el destinatario es externo-
	Tipo VARCHAR(20),  -- si es interno o externo 
	FOREIGN KEY (ID_documento) REFERENCES documentos(ID) ON DELETE CASCADE,
	FOREIGN KEY (ID_abogado) REFERENCES abogados(ID)
);

## El historial del prestamo pertenece a el/los material/es
-- Prestamos de materiales de la biblioteca (1:M)
...
## No hay relacion si se elimina alguna de las dos (el conocimiento o el experto)
...
-- Así como la tabla de arriba, no hay relacion si se elimina alguna de las dos
...
## NO hay relación si se elimina alguna de las dos (conocimiento o precedente) (ON DELETE CASCADE)
-- Un conocimiento puede estar respaldado por muchos precedentes y	
-- un precedente puede aplicar a muchos conocimientos (M:M)
...
INSERT INTO tipos_audiencia (Tipo_audiencia) 
VALUES ('Inicial'), ('Pruebas'), ('Alegatos'), ('Sentencia');
...
VALUES ('Contrato'), ('Demanda'), ('Recurso'), ('Informe'), ('Dictamen');

INSERT INTO estados_documento (Estado) 
VALUES ('Borrador'), ('Revision'), ('Final');
